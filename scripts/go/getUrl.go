...
func getUrlHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.DepartureAirport == "" || req.DestinationAirport == "" || req.StartDate == "" || req.ReturnDate == "" || req.TripType == "" {
	...
	// One-way trips don't use a return date
	effectiveReturnDate := returnDate
	if tripType == flights.OneWay {
		effectiveReturnDate = time.Time{}
	}

	url, err := generateFlightUrl(startDate, effectiveReturnDate, req.DepartureAirport, req.DestinationAirport, stops, class, tripType)
	if err != nil {
...
func generateFlightUrl(startDate, returnDate time.Time, src, dst string, stops flights.Stops, class flights.Class, tripType flights.TripType) (string, error) {
